...
import ModelsService from "@/Services/ModelService.js";
...
const modelsLength = ref(0);
const dataLoadedPerPage = ref([]);
const currentPage = ref(1);
const searchQuery = ref("");
const totalNumberOfPages = () => {
  return Math.ceil(filteredModels.value.length / itemsPerPage.value);
};
const getDataPerPage = computed(() => {
...
const updateSortingDirection = (newSortingDirection) => {
  currentSortDir.value = newSortingDirection;
};
const updateDataLoadedPerPage = (page) => {
